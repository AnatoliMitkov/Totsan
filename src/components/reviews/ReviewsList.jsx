...
function reviewerLabel(review) {
  const tail = String(review.clientId || '').slice(-4).toUpperCase()
  return tail ? `Клиент ${tail}` : 'Клиент от Totsan'
}
...
export default function ReviewsList({
  ...
  emptyText = 'Още няма публикувани отзиви.',
}) {
  ...
  return (
    <section className="mt-10 rounded-3xl border border-line bg-paper p-5 md:p-7">
      ...
      {status === 'ready' && reviews.length === 0 && <div className="mt-6 rounded-2xl border border-dashed border-line p-5 text-center text-sm text-muted">{emptyText}</div>}
    ...
    <article className="rounded-2xl border border-line bg-soft p-4 md:p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-1 text-xs text-muted">{reviewerLabel(review)} · Проверена поръчка · {formatReviewDate(review.createdAt)}</div>
        </div>
        {userId && userId !== review.clientId && (
          <button type="button" onClick={() => setOpen((current) => !current)} className="inline-flex items-center gap-1 rounded-full border border-line bg-paper px-3 py-1 text-xs font-medium text-muted transition hover:text-ink">
            <Flag size={14} /> Сигнал
          </button>
        )}
      </div>
