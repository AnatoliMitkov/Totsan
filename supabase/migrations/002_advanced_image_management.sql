-- supabase/migrations/002_advanced_image_management.sql
...
create table if not exists public.image_metadata (
  ...
  bucket              text not null, -- 'profile-images', 'project-media', etc.
  path                text not null unique, -- Full storage path
  category            text not null check (category in ('profile', 'banner', 'portfolio', 'service', 'project', 'temp')),
  type                text, -- 'main', 'thumb', 'og', 'cover', 'image-001', 'before-001', etc.
  ...
  variant             text, -- 'main', 'blur', 'tablet', 'mobile', 'thumb', 'og'
  version_number      integer, -- For version history
...
-- ============================================================================
-- MATERIALIZED VIEW: Image Storage Stats
-- ============================================================================
...
-- ============================================================================
-- MATERIALIZED VIEW: Storage by Category
-- ============================================================================
...
-- ============================================================================
-- RPC FUNCTION: Get profile image paths
-- ============================================================================
...
-- ============================================================================
-- RPC FUNCTION: Storage usage for profile
-- ============================================================================
...
-- ============================================================================
-- SCHEDULED CLEANUP JOBS (via pg_cron if available)
-- ============================================================================

-- Daily cleanup of temp uploads older than 48 hours
-- select cron.schedule('cleanup-temp-uploads-daily', '0 2 * * *', $$
--   delete from public.image_metadata
--   where category = 'temp'
--   and created_at < now() - interval '48 hours'
-- $$);

-- Weekly cleanup of old versions (keep last 10)
-- select cron.schedule('cleanup-old-versions-weekly', '0 3 * * 0', $$
--   delete from public.image_metadata
--   where not is_current
--   and created_at < now() - interval '90 days'
--   and (select count(*) from image_metadata where profile_id = image_metadata.profile_id and not is_current) > 10
-- $$);
